# Marked.js (문서 갱신: 2025-12-19)

2024-09-13 이후 marked는 세 번의 큰 변곡을 겪었습니다.

- 15.0.0: HTML 이스케이프 위치 변경
- 16.0.0: CJS 제거 + `marked.min.js` 제거 + 최소 Node 20으로 상향(ESM 시대로 전환)
...
## 1. 릴리스 타임라인 (공식 GitHub Releases 기준, UTC)
...
| 2025-06-27 | 16.0.0  | 메이저 | CJS 빌드 제거, `marked.min.js` 제거, 최소 Node 20 (브레이킹) |
...
| 16.0.0 | 브레이킹 | CJS 빌드 제거, `marked.min.js` 제거, Node 20+ | `require('marked')` 깨짐, `marked.min.js` → `./lib/marked.umd.js` | ESM import 전환, UMD는 `./lib/marked.umd.js` 사용                               |
...
- CJS 빌드 제거, `marked.min.js` 제거 (`./lib/marked.umd.js` 사용), 최소 Node 20.
...
- [GitHub: markedjs/marked Releases](https://github.com/markedjs/marked/releases)
