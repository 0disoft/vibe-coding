# Bun (문서 갱신: 2025-12-19)

2024년 하반기부터 2025년 4분기까지 Bun의 행보는 명확합니다. 단순한 "빠른 Node.js 대체재"를 넘어, 데이터베이스, 프론트엔드 빌드, 보안, 테스팅 도구를 모두 내재화한 **All-in-One 인프라**로 거듭나고 있습니다.
...
1. **Zero-Dependency Data Layer:** PostgreSQL, MySQL, SQLite를 `Bun.SQL`로, Redis/Valkey를 `Bun.redis`로 런타임 레벨에서 내장했습니다. 외부 드라이버 의존성을 제거하고 I/O 성능을 극대화하려는 전략입니다.
...
| **v1.2.0**  | **Native Data Layer**    | **[데이터]** `Bun.SQL`(PostgreSQL/MySQL/SQLite)의 등장은 외부 DB 드라이버(pg, mysql2 등) 제거를 의미합니다. `import { sql } from "bun"` 패턴을 표준으로 채택하십시오.                  |
| **v1.2.5**  | **Test Enhancement**     | **[테스트]** `test.failing()` 지원 추가로 실패 예상 테스트 패턴을 명시적으로 작성할 수 있습니다.                                                                                           |
| **v1.2.9**  | **Redis Integration**    | **[데이터]** `Bun.redis` 내장 Redis 클라이언트 최초 도입. 별도의 Redis 드라이버 설치 없이 Redis/Valkey 다이렉트 연결 가능.                                                          |
| **v1.2.20** | **Type-Level Testing**   | **[테스트]** `expectTypeOf` 지원 추가로 타입 레벨 테스트가 가능해졌습니다.                                                                                                                                    |
| **v1.2.21** | **Secrets Management**   | **[보안]** `Bun.secrets` 도입. OS 키체인(macOS Keychain/libsecret/Windows Credential Manager) 연동으로 비밀키 관리.                                                         |
...
- **Database:** `pg`나 `mysql2` 설치를 금지하고 `Bun.SQL`(PostgreSQL/MySQL/SQLite)과 `Bun.redis`(Redis/Valkey)를 표준으로 채택하십시오. ORM이 필요하다면 Drizzle ORM 등과 조합하여 Bun Native 드라이버를 활용하는 것이 성능상 유리합니다.
...
- **Testing:** Jest를 `bun:test`로 교체하는 것이 가장 ROI가 높습니다. v1.2.1의 인라인 스냅샷, v1.2.5의 `test.failing`, v1.2.20의 `expectTypeOf` 지원으로 마이그레이션 장벽이 거의 사라졌습니다.
...
Bun은 이제 단순한 'JS 런타임'이 아닙니다. 데이터, 보안, 빌드, 테스트를 모두 아우르는 **"인프라 스트럭처로서의 자바스크립트"**를 지향하고 있습니다. 팀의 기술 스택을 단순화(Simplification)하고 싶다면, 지금이 Bun v1.3.5+ 도입을 진지하게 고려해야 할 시점입니다.
...
- v1.3.4 이후 최신 릴리스. 안정성 및 버그 수정 포함.
...
> **참조:** [Bun Releases](https://bun.com/blog)
