...
├─────────────────────────────────────────────────────────┤
│  원시 토큰 (Primitive)                                   │
│  예: --raw-color-blue-500, --raw-color-gray-100, --spacing-4 │
└─────────────────────────────────────────────────────────┘
```

**토큰 참조 규칙:**

- **원시 토큰 (Primitive)**: 다른 토큰을 참조하지 않음 (리터럴 값만)
- **시맨틱 토큰 (Semantic)**: 원시 토큰을 참조, 테마 전환은 여기서 처리
- **컴포넌트 토큰 (Component)**: 시맨틱 토큰을 참조, 컴포넌트 내부 미세 조정용
- **UI 구현**: 컴포넌트는 원시 토큰을 직접 참조하지 않음
...
| 원시 | 원시 토큰 값 | ⚠️ RFC 필수 | 브랜드 리뉴얼 시 팀 합의 + RFC 문서화 필수 |
...
| 중립 | `--color-neutral-50` ~ `--color-neutral-950` | 텍스트, 경계, 표면 |
| 오버레이 | `--color-overlay`, `--color-scrim` | 모달 배경, hover overlay (알파 포함) |
| 온-컬러 | `--color-on-primary`, `--color-on-surface`, `--color-on-error` | 색상 위 텍스트/아이콘 |
...
**4px 베이스 시스템(0.25rem 단위)** 을 권장합니다. 모바일 우선 시대에 8px 베이스는 간격이 과도해지기 쉽습니다. (전제: `1rem = 16px`)
...
| 카테고리 | 예시 |
|----------|------|
| 테두리 | `--radius-sm`, `--radius-md`, `--border-width`, `--border-color` |
| 그림자/엘리베이션 | `--shadow-sm`, `--shadow-md`, `--elevation-1`, `--elevation-2` |
| 전환 | `--duration-fast`, `--duration-normal`, `--easing-ease-out`, `--motion-scale` |
| z-index | `--z-dropdown`, `--z-modal`, `--z-toast` |
| 브레이크포인트 | `--breakpoint-sm`, `--breakpoint-md`, `--container-max` |
| 사이징 | `--size-icon-sm`, `--size-icon-md`, `--size-control-height` |
| 투명도 | `--opacity-disabled`, `--opacity-hover` |
...
```css
:root {
  /* 원시 토큰 (--raw-color-* 접두사로 통일) */
  --raw-color-blue-500: oklch(55% 0.15 250);
  --raw-color-blue-600: oklch(48% 0.15 250);
  
  /* 시맨틱 토큰 */
  --color-primary: var(--raw-color-blue-500);
  --color-primary-hover: var(--raw-color-blue-600);
}
...
> [!NOTE] OKLCH는 테마/명도 조절에 유리합니다. **PostCSS OKLCH 플러그인**으로 빌드 타임에 sRGB 자동 변환하면 호환성 걱정 없이 사용할 수 있습니다.
...
|-----------|----------|------|
| AA (일반) | 4.5:1 | 일반 텍스트 |
| AA (대형) | 3:1 | 18pt (≈24px) 이상 또는 14pt Bold (≈18.66px) |
| AAA | 7:1 | 가독성 최우선 |
...
| 항목 | 기준 |
|------|------|
| 터치 타깃 | 권장 44×44px (AAA), 최소 24×24px (WCAG 2.2 AA) |
| 키보드 | Tab 순서, 포커스 표시, ESC로 닫기 |
| 포커스 링 | 색상 + 두께(최소 2px) + 오프셋(최소 2px) + 대비(3:1 이상) 토큰화 |
| 모션 | `prefers-reduced-motion` 대응 필수 - 단순 0s가 아닌 **페이드 전환 등 대체 경험** 제공 |
| 고대비 | `prefers-contrast: more`, `forced-colors` (Windows 고대비 모드) 대응 |
| 확대 | zoom 200%~400% 테스트, 텍스트 리플로우 확인 |
| 색상 원칙 | **색상만으로 정보 전달 금지** - 아이콘, 텍스트, 패턴 병행 |
...
타이포그래피 품질을 좌우하는 세부 토큰입니다.

| 토큰 | 용도 |
|------|------|
| `--letter-spacing-tight` | 제목 등 대형 텍스트 (-0.02em) |
| `--letter-spacing-normal` | 본문 (0) |
| `--letter-spacing-wide` | 대문자/캡션 (0.05em) |
| `--paragraph-spacing` | 문단 간 간격 (1em~1.5em) |

**줄바꿈 정책:**

| 언어 | 권장 설정 |
|------|----------|
| 한글 | `word-break: keep-all` (단어 단위) - **모바일 좋은 화면에서는 `normal` 허용** |
| 영문 | `overflow-wrap: break-word` (긴 단어 분리) |
| 숫자 | `font-variant-numeric: tabular-nums` (표 정렬용) |
...
>
> - 변형 축은 2~3개까지 (size + variant 또는 size + intent + stateful)
> - 3축 초과 시 분리 컴포넌트를 고려하세요
...
6. **컴포넌트별 접근성 책임**: 핵심 컴포넌트가 보장해야 할 접근성 규격
...
| 야간 | 의사 로케일(30~80% 텍스트 팽창), RTL, 시각 회귀 | 매일 |
...
- [ ] 복수형/숫자 단위 처리 확인
- [ ] 날짜/시간/숫자 포맷이 로케일 따름
- [ ] 텍스트 팽창(30~40%) 시 레이아웃 유지

### 핵심 컴포넌트 접근성 책임

컴포넌트 개발 시 반드시 보장해야 할 접근성 규격입니다.

| 컴포넌트 | 책임 |
|----------|------|
| Dialog/Modal | focus trap, ESC 닫기, `aria-modal`, return focus |
| Dropdown/Menu | roving tabindex, 화살표 내비게이션, `aria-expanded` |
| Tabs | 화살표 내비게이션, `aria-selected`, `aria-controls` |
| Tooltip | `aria-describedby`, hover + focus 트리거, 타임아웃 충분히 |
| Toast/Alert | `role="alert"` 또는 `role="status"`, 자동 닫힘 시간 충분히 |
...
3. **코어 컴포넌트 3개**: **Button을 먼저 완성해서 바로 제품에 박으세요.** Button이 합의되면 Input, Textarea, Select는 거의 복붙 수준으로 빠릅니다. Modal/Dialog는 그 다음.
...
표준화는 도로를 까는 일이고 자유도는 건물을 짓는 일입니다. 균형은 "도로만 강하게, 건물은 느슨하게"에서 나옵니다.

**3단 레이어:**

| 레이어 | 설명 | 표준화 수준 |
|--------|------|------------|
| **토대** | 토큰, 간격, 타이포, 색, 접근성 규칙 | 필수 표준화 |
| **도구** | 버튼, 인풋, 모달, 폼 검증, 로딩/에러 패턴 | 강하게 표준화 |
| **도메인** | 결제 화면, 콘텐츠 카드, 검색 그리드, 회원가입 퍼널 | 가이드만 |

**표준화 결정 5가지 질문 (예 3개 이상이면 표준화 후보):**

1. 한 달에 3번 이상 다시 만들고 있나?
2. 만들 때마다 버그나 접근성 문제가 반복되나?
3. 한번 깨지면 영향 범위가 넓나?
4. 디자인 일관성이 사용자 신뢰에 직결되나?
5. 다른 화면에서 모양은 달라도 행동은 같아야 하나?

> [!IMPORTANT]
> "행동이 같아야 한다"가 가장 강력합니다. 표준화는 외형보다 행동에서 더 큰 돈을 벌어요.

**실전 기준선:**

| 구분 | 대상 |
|------|------|
| 반드시 표준화 | 토큰, 접근성 규칙, 폼 컨트롤, 피드백 컴포넌트, 라우팅/권한 패턴 |
| 가이드만 | 카드 레이아웃, 리스트 그리드, 콘텐츠 블록, 페이지 템플릿 |
| 표준화 금지 | 제품 고유 핵심 화면, 성장 실험, 차별화 목적 UI |
...
> [!TIP]
> **원칙은 포스터가 아니라 의사결정 장치입니다.** 원칙이 팀의 말다툼을 줄여주지 못한다면, 그 원칙은 장식품이에요.
...
확실한 대형 투자로 뛰기 전에, 경량 시스템이 가장 자주 이깁니다. 임계점을 "빨리 만나게" 만들어주니까요.

- 시스템 단위를 작게: "전면 도입" 대신 가장 많이 반복되는 1개 흐름만 먼저
- 즉시 보상 포인트: 도입 첫 주에 바로 이득이 보이는 자동화
- 되돌리기 쉽게: 마이그레이션 비용이 크면 경량 버전으로 시작

> [!TIP]
> 경량이 이긴 뒤에, 완전 시스템으로 진화시키면 됩니다.
...
신규 컴포넌트/토큰 제안 시 아래 3가지 기준으로 판단합니다.

| 기준 | 질문 |
|------|------|
| **Consistency** (일관성) | 기존 패턴과 얼마나 일치하는가? |
| **Coverage** (범위) | 몇 개 팀/화면에서 필요한가? (3개 이상 시 승격 고려) |
| **Cost** (비용) | 구현/유지보수 비용 대비 효과가 큰가? |

> [!TIP]
> "신규 컴포넌트 제안 시 3C 기준 점수표를 첨부하세요"라고 명시하면 의사결정이 빨라집니다.
...
새 컴포넌트/패턴은 처음부터 표준이 아닙니다.

1. **실험 단계**로 시작
2. **2개 이상 화면**에서 재사용 확인
3. **유지보수 비용** 낮음 확인
4. 정식 승격 또는 **폐기**

표준 라이브러리를 박물관으로 만들지 않는 장치입니다.
...
`exceptions.md` 파일을 두고 예외 목록을 한 줄씩 쌓습니다. 날짜, 규칙, 만료 조건, 상태만 적는 인덱스예요. 예외가 늘어나는 속도를 눈으로 보게 만드는 장치입니다.
...
| 도구 | 용도 |
|------|------|
| **Figma Variables** | 네이티브 변수 기능 → Single Source of Truth (권장) |
| **Figma Tokens** | 커뮤니티 플러그인 → 레거시 프로젝트용 |
| **Style Dictionary** | 토큰 변환 (CSS, JS, iOS, Android) |
| **PostCSS OKLCH** | OKLCH→sRGB 빌드 타임 변환 |
| **Contrast Checker** | WCAG 2.2 대비 검증 |
| **Type Scale** | 타이포그래피 스케일 생성기 |

### 토큰 파이프라인 예시

토큰 동기화를 자동화하지 않으면 몇 달 만에 Figma와 코드가 어긋납니다.

```plaintext
Figma 변수 → tokens.json (Figma Tokens 익스포트)
  ↓
Style Dictionary 변환
  ↓
├── tokens.css (CSS 변수)
├── tokens.js (JS 객체)
└── tokens.xml / tokens.swift (네이티브 앱용, 필요시)
```

> [!TIP]
> CI에서 Figma→tokens.json 자동 동기화 스크립트를 걸어두면 드리프트를 조기에 잡을 수 있습니다.
>
> [!IMPORTANT]
> **W3C DTCG 표준 준수:** 토큰 정의 파일은 W3C Design Tokens Community Group 표준 포맷(`$value`, `$type`, `$description`)을 따릅니다. 2025년 10월 Stable 버전 도달로 업계 표준이 되었으며, 이를 따르면 향후 툴 교체 비용이 획기적으로 줄어듭니다.

## 아이콘 시스템

색상 다음으로 변경 빈도가 높고 개발-디자인 간 마찰이 적은 영역입니다.

### 규격

| 항목 | 기준 |
|------|------|
| viewBox | 24px × 24px (표준), 16px/20px 변형 허용 |
| 색상 | `fill="currentColor"` → 시맨틱 토큰 상속 |
| stroke | 1.5px~2px 통일 |
| 코너 | 라운드 2px 또는 샤프 통일 |

### 아이콘 네이밍

형태 기반 네이밍을 권장하고, 행위 기반은 알리아스로 처리합니다.

| 권장 | 비권장 |
|------|----------|
| `icon-chevron-right` | `icon-next` (행위 기반) |
| `icon-hamburger` | `icon-menu` (행위 기반) |
...
Figma 컴포넌트 → SVG 익스포트
  ↓
SVGO 최적화 (viewBox 통일, 불필요 속성 제거)
  ↓
React/Vue/Svelte 컴포넌트 자동 변환 (svgr, unplugin-icons 등)
```

> [!TIP]
> 아이콘 변경 시 PR에 전/후 대비 스냅샷을 포함하면 시각적 깨짐을 조기에 발견할 수 있습니다.
