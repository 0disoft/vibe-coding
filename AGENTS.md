...
- **다국어 파일 생성 전략**: 대량의 언어 파일(i18n)을 생성해야 할 경우, 처음부터 모든 언어를 한 번에 생성하지 않습니다. 우선 한국어(ko), 영어(en), 그리고 RTL(Right-to-Left) 레이아웃 테스트를 위한 아랍어(ar) 등 3개 주요 언어 파일을 먼저 생성하여 구조와 렌더링을 테스트하고, 최종적으로 검증이 완료된 후에 나머지 언어 파일을 생성합니다. 이는 오류가 모든 파일로 확산되는 것을 방지하고 수정 비용을 최소화하기 위함입니다.
- **터미널 출력 최소화**: 명령어 실행 결과가 길 경우(약 500자 이상) 터미널에 전체 내용을 출력하지 않습니다. 긴 결과는 **사용자 요청이 있을 때만** `.vibe-coding/.reports.txt`에 저장하고 경로만 안내하거나, 기본은 `head`/`tail` 등으로 일부만 표시합니다. **단, 사용자가 “읽어봐”처럼 읽기만 요청한 경우에는 저장·요약·출력을 하지 말고 내부적으로 확인 후 대기합니다.** 불필요한 터미널 출력은 랙과 메모리 부족을 유발합니다.

## 로깅/디버깅 규칙 (DEV vs preview/CI/E2E)

SvelteKit/Vite 환경에서 `import.meta.env.DEV`는 **빌드 타임 상수**이므로,
Playwright `webServer`가 쓰는 `bun run build && bun run preview`(= production build/preview)에서는 항상 `false`입니다.

따라서 아래 상황에서는 `import.meta.env.DEV`만으로는 디버깅이 불가능해질 수 있습니다:

- **E2E(Playwright)에서만 500이 발생**하고, 로컬 `bun dev`에서는 재현이 어려운 경우
- `vite preview` 또는 CI에서만 SSR 에러가 터지는 경우

운영 원칙:

- “상세 스택/에러 객체” 로그는 기본적으로 **DEV에서만** 출력합니다.
- 단, E2E/preview/CI 디버깅을 위해 **런타임 환경변수 플래그로 opt-in**할 수 있어야 합니다.
  - 예: `VIBE_LOG_SERVER_ERRORS=1`처럼 “서버 에러 상세 출력”을 명시적으로 켜는 플래그를 사용
  - 이때도 보안상 민감 정보(토큰/쿠키/개인정보)는 절대 로그에 남기지 않습니다.

## 실행 명령 규칙 (bun 우선)

이 저장소는 `package.json` 스크립트를 단일 진입점으로 사용합니다.

- 개발 서버 실행은 `vite dev`를 직접 치지 말고 **`bun dev`**를 사용합니다.
- 프리뷰 서버도 직접 `vite preview`를 치지 말고 **`bun run preview`**를 사용합니다.
- 이유:
  - 스크립트에 포함된 사전/사후 작업(예: i18n 생성, postbuild 등)과 옵션이 누락되는 것을 방지
  - 팀/에이전트 간 실행 명령을 통일해 재현성을 확보

## 커뮤니케이션 규칙

- 읽기 요청만 있을 때: 사용자가 파일을 읽어보라고만 하고 별도 행동을 요구하지 않으면, 내용을 요약하거나 추가 행동을 시도하지 않고 대기합니다.
- 간결한 보고: 진행 상황, 발견한 이슈, 다음 행동을 한 번에 전달합니다.
- 한국어 기본: 모든 답변, 설명, 코드 주석은 한국어로 작성하며, 다른 언어가 필요하면 사용자에게 먼저 확인합니다.
- 경로 링크 가독성: 터미널/채팅에서 경로가 자동 링크로 인식되므로, 경로 뒤 조사/어미(예: `~에`, `~에서`, `~의`)는 **붙여 쓰지 말고 띄어쓰기**로 분리합니다. 예: `AGENTS.md` 에, `.vibe-coding/TROUBLE/SOLVED.md` 에서
- **경로 호환성 주의**: 윈도우/리눅스 환경 차이(`\` vs `/`)로 인한 검증 실패를 막기 위해 항상 `normalize`로 구분자를 통일해야 합니다. 특히 자동 생성되는 문서(매니페스트 등)에는 **로컬 절대 경로가 포함되지 않도록 반드시 `path.relative`를 사용해 프로젝트 루트 기준 상대 경로로 저장**해야 합니다.
...
| ⚠️ 위험 | 구현 가능하나 위험 | 사이드 이펙트가 우려되는 제안 |
| 🔀 대안 제시 | 다른 방식으로 해결 | 제안 대신 더 나은 방법이 있을 때 |
| 📝 문서화 | 코드 아닌 문서로 해결 | 구현보다 가이드 문서가 적절할 때 |
...
- 테스트는 "존재 확인"을 넘어 **실제 동작을 검증**해야 합니다. 버튼이 있다고 통과하는 게 아니라, 클릭 시 의도한 함수가 호출되는지까지 확인합니다.
- **단일 소스 원칙**: 매직 넘버(예: 언어 파일 20개)는 하드코딩하지 않고, 프로젝트의 공식 소스(예: `$lib/paraglide/runtime`의 `locales`)를 import하여 사용합니다.
- **경로는 `import.meta.url` 기반**: `process.cwd()`는 CI/모노레포 환경에서 불안정합니다. 테스트 파일 기준 상대 경로를 사용합니다.
- **BOM 방어**: JSON 파싱 시 `.replace(/^\uFEFF/, '')`로 BOM을 제거합니다. 일부 편집기/번역툴이 BOM을 삽입할 수 있습니다.

#### 컴포넌트 테스트 (`vitest-browser-svelte`)

- **접근성 이름 기반 선택**: `page.getByRole('button', { name: /copy code/i })`처럼 `aria-label`이나 visible text를 사용합니다. CSS 선택자보다 깨지기 어렵습니다.
- **`page.locator()` 사용 불가**: `vitest-browser`의 `page`는 Playwright와 다릅니다. `getByRole`, `getByText`, `getByTestId` 등을 사용합니다.
- **API 모킹**: `navigator.clipboard` 같은 브라우저 API는 테스트에서 모킹하고, 테스트 후 원래 값으로 복원합니다.
- **cleanup**: `afterEach`에서 `unmount`를 호출하여 테스트 간 DOM 격리를 보장합니다.

#### i18n 메시지 테스트

- **중첩 키 검증**: 최상위 키만 비교하면 깊은 키 누락을 놓칩니다. `flattenKeys` 헬퍼로 점 표기법(`nav.home`)으로 펼쳐서 비교합니다.
- **플레이스홀더 일치 검증**: `{name}`, `{count}` 같은 플레이스홀더가 모든 언어에서 동일한지 확인합니다. 누락된 플레이스홀더는 런타임 오류를 유발합니다.
- **파일-로케일 1:1 매칭**: `messages/*.json` 파일과 `SUPPORTED_LOCALES` 배열이 정확히 일치하는지 검증합니다. 불필요한 파일이나 누락된 파일을 감지합니다.
...
- 순서 있는 목록 항목은 상황에 따라 `1.` 또는 `1. 2. 3.` 순차 번호를 사용합니다. (`MD029`)
- 테이블 컬럼 구분선은 가급적 양옆에 공백을 두는 스타일(`| --- |`)을 권장하지만, 린트 규칙(`MD060`)으로는 강제하지 않습니다.
- 줄 끝에는 불필요한 공백을 남기지 않습니다. (`MD009`)
- 한 줄 길이 제한(`MD013`)은 가독성과 편집 편의를 위해 적용하지 않습니다.
- 순서 없는 목록의 들여쓰기는 항상 2칸을 사용합니다. (`MD007`)
- 마크다운 굵게(`**`) 렌더링 깨짐 대표 패턴:
  - 닫힘 마커(`**`) 직전에 특수문자(`)`, `"`, `:`, `.` 등)가 있고, 닫힘 마커 바로 뒤에 공백 없이 한글 조사/어미가 붙으면 일부 파서에서 닫힘을 인식하지 못해 마커가 그대로 노출됩니다.
  - 영어와 한글이 띄어쓰기 없이 이어지는 경계에 `**`가 위치해도 동일한 문제가 발생합니다.
  - 해결책 (우선순위순):
    1. 특수문자(괄호, 따옴표 등)를 굵게 바깥으로 빼기 (권장)
    2. 조사/어미를 굵게 안에 포함하기
    3. 굵게 뒤에 공백 추가하기
  - 금지 예:
    - `**이메일과 채팅으로 명확한 취소 의사(Unwillingness)**를` → 깨짐
    - `"**서비스 거부 의사 표시를 완료했다**"고` → 깨짐
  - 권장 예:
    - `**이메일과 채팅으로 명확한 취소 의사**(Unwillingness)를` → 괄호를 밖으로
    - `"**서비스 거부 의사 표시를 완료했다**"고` → 따옴표를 밖으로
...
## 슬라이딩 윈도우 규칙 (Count & Memory Checkpoint)

매 답변 끝에서 직전 답변 마지막 줄의 `[Count=숫자]`를 찾아 1을 더해 새 마지막 줄에 단독 표기하며, 없으면 `[Count=1]`로 시작합니다. `[Count=N]`은 반드시 전체 출력의 가장 마지막 줄에 위치해야 합니다.

N이 10의 배수일 때는 Count 바로 윗줄에 Priorities 내용 3줄을, 그 위에 제목 `Memory Checkpoint`를, 그 위에 구분선 `---`를 순서대로 배치합니다.

Priorities 작성 규칙:

- 카테고리 없이 대화 흐름 유지에 필수적인 최상위 정보만 선별합니다.
- 각 줄은 80자 이내, 총 3줄 합산 240자 이내로 요약합니다.
- 사실이 아닌 추정 내용에는 문장 끝에 '(추정)'을 명시합니다.

이 슬라이딩 윈도우 규칙을 통해 주제가 바뀌어도 핵심 맥락을 영구히 이월합니다.
