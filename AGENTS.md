...
- 볼드(`**`) 닫힘 직후 비라틴 문자(한글 등)가 공백 없이 바로 오면 파싱 오류가 발생합니다. 특히 **볼드 내부가 특수문자(`:`, `)`, `.` 등)로 끝날 때** 이 문제가 자주 나타납니다.
  - ❌ `**무료:**&#8203;로` → 파싱 실패, `**`가 그대로 노출됨
  - ❌ `**결정(Decision)**&#8203;이` → 파싱 실패
  - ✅ `**무료:** 로` 또는 `**무료**로` → 공백 추가로 해결
  - ✅ `**결정(Decision)**&#8203;&#8203;이` → Zero-Width Space(`&#8203;`)로 해결
