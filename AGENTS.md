...
- **터미널 출력 최소화**: 명령어 실행 결과가 길 경우(약 500자 이상) 터미널에 전체 내용을 출력하지 않습니다. 긴 결과는 `.vibe-coding/.reports.txt`에 저장하고 경로만 안내하거나, `head`/`tail` 등으로 일부만 표시합니다. **단, 사용자가 “읽어봐”처럼 읽기만 요청한 경우에는 저장·요약·출력을 하지 말고 내부적으로 확인 후 대기합니다.** 불필요한 터미널 출력은 랙과 메모리 부족을 유발합니다.
