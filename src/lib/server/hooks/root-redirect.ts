...
import { getLocaleCookieSetOptions } from '$lib/shared/utils/locale';
...
export const handleRootRedirect: Handle = async ({ event, resolve }) => {
	...
	if (event.url.pathname === '/' && !hasQuery) {
		...
		if (cookie && isLocale(cookie)) {
			// 쿠키 옵션(만료/secure/samesite/path)을 SSOT로 강제
			event.cookies.set(cookieName, cookie, getLocaleCookieSetOptions(event.url));
		...
		if (preferredLocale && preferredLocale !== baseLocale && isLocale(preferredLocale)) {
			...
			// 최초 감지된 선호 언어를 쿠키로 고정 (다음 방문부터는 쿠키 우선)
			event.cookies.set(cookieName, preferredLocale, getLocaleCookieSetOptions(event.url));
