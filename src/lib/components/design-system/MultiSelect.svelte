<script lang="ts">
  ...
  let selectedOptions = $derived(options.filter((o) => values.includes(o.value)));
  let isPlaceholder = $derived(selectedOptions.length === 0);

  let visibleChips = $derived(selectedOptions.slice(0, Math.max(0, maxChips)));
  let overflowCount = $derived(Math.max(0, selectedOptions.length - visibleChips.length));
...
<DsDropdown
  ...
  itemSelector={'[role="menuitemcheckbox"]'}
>
