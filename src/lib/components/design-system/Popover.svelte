<script lang="ts">
  ...
  function toggle(): void {
    if (disabled) return;
    const next = !isOpen;
    if (next) previousActiveElement = (document.activeElement as HTMLElement) ?? null;
    setOpen(next);
  ...
  $effect(() => {
    ...
    if (side === "bottom" || side === "top") {
      const baseTop = side === "bottom" ? rect.bottom + offset : rect.top - panelRect.height - offset;
      top = baseTop;
      ...
      const baseLeft = side === "right" ? rect.right + offset : rect.left - panelRect.width - offset;
      left = baseLeft;
...
>
  ...
  {#if isOpen}
    <div
      bind:this={panelEl}
      id={panelId}
      class={["ds-popover-panel ds-elevation-2", panelClass].filter(Boolean).join(" ")}
