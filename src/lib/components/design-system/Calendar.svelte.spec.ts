...
describe('DsCalendar', () => {
	...
	it('PageDown으로 월 이동이 가능해야 한다', async () => {
		...
		await expect
			.element(page.getByTestId('ds-calendar-caption'))
			.toHaveTextContent('February 2025');
	});
	...
	it('min/max 범위 밖 날짜는 disabled여야 한다', async () => {
		...
		await expect
			.element(page.getByTestId('ds-calendar-day-2025-01-09'))
			.toHaveAttribute('disabled', '');
		await expect
			.element(page.getByTestId('ds-calendar-day-2025-01-21'))
			.toHaveAttribute('disabled', '');
	});
});
