<script lang="ts">
  ...
  let items = $derived.by(() => {
    return locales.map((locale) => {
      ...
      const keywords = [info.selfName, info.englishName, locale].join(" ").toLowerCase();
...
<div {...rest} class={["inline-flex", className].filter(Boolean).join(" ")}>
  ...
  <DsDropdown
    ...
    menuClass="!min-w-fit w-max max-h-[360px] overflow-y-auto thin-scrollbar"
        ...
        >
          {#snippet children()}
            <span class="flex min-w-0 items-center justify-between gap-3">
              ...
              <span class="text-xs text-muted-foreground">{item.locale.toUpperCase()}</span>
            </span>
          {/snippet}
        </DsDropdownItem>
      {/each}
