<script lang="ts">
  ...
  import type { PaymentMethodType, PaymentOption, UnavailabilityReason } from "./payment-types";
  ...
  let {
    option = null,
    title,
    emptyText = m.payment_details_select_prompt(),
  ...
  let checkResult = $derived(option ? checkPaymentOptionAvailability(option) : null);
  let badgeLabel = $derived(
    option?.badge ?? (option?.recommended ? m.payment_recommended() : undefined),
  );
  ...
  let availabilityNote = $derived.by(() => {
    ...
    return checkResult.message ?? getReasonLabel(checkResult.reason) ?? option.availabilityNote;
  });
...
<div {...rest} class={rootClass} aria-live="polite">
  {#if option}
    ...
    {#if availabilityNote}
      <div class="ds-payment-details-alert text-helper text-destructive flex items-start gap-1.5 mt-2 font-medium">
      ...
      {#if metaItems.length}
        ...
        >
          {#each metaItems as item (item)}
            <li class="text-xs bg-muted/50 px-2 py-0.5 rounded-full border border-border">
              {item}
            </li>
          {/each}
        </ul>
          ...
          <ul class="ds-payment-details-list space-y-2 p-0 list-none">
            {#each option.details as item (item)}
              <li class="text-body-secondary flex items-start gap-2">
                <DsIcon name="check-circle-2" size="xs" class="text-success mt-1 shrink-0" />
                <span>{item}</span>
              </li>
            {/each}
          </ul>
    ...
    {/if}
  {:else}
    <div class="ds-payment-details-empty text-helper text-muted-foreground py-8 text-center border-2 border-dashed border-border rounded-lg">
      {emptyText}
    </div>
  {/if}
</div>
