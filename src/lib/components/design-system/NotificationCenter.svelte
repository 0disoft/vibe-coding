...
<div {...rest} class={["ds-notification-center", className].filter(Boolean).join(" ")}>
  ...
  <DsTabs value={filterState.value} onValueChange={(v) => (filterState.value = v as "all" | "unread")}>
      ...
      <ul class="ds-notification-list" aria-label="Notification list">
        ...
        {:else}
          {#each visibleItems as item (item.id)}
            <li class="ds-notification-item" data-read={item.read ? "true" : undefined} data-disabled={item.disabled ? "true" : undefined}>
              <button
                type="button"
              ...
              >
                <span class="ds-notification-icon" aria-hidden="true">
                  <DsIcon name={intentIcon[item.intent ?? "neutral"] ?? "bell"} size="sm" />
                </span>
