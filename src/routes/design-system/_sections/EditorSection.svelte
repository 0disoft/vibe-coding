<script lang="ts">
	...
	import type {
		EditorBlockType,
		EditorCommand,
	} from "$lib/components/design-system/EditorToolbar.svelte";
...
<section id="ds-editor" class="space-y-4">
	...
	<DsCard class="space-y-4">
			...
			<div class="rounded-lg border border-border bg-background p-3 space-y-3">
				...
				<div class="rounded-md border border-border bg-surface p-3 relative">
					...
					<div
						...
						role="textbox"
						tabindex="0"
						class="ds-textarea ds-focus-ring min-h-[160px] whitespace-pre-wrap outline-none"
						oninput={(e) =>
							(contentValue = (e.currentTarget as HTMLElement).innerText)}
					>
						{contentValue}
					</div>
		...
		<div class="grid gap-4 md:grid-cols-2">
			<DsField
				label="Link (demo)"
				helpText="링크 커맨드는 보통 URL 입력 다이얼로그/Popover로 연결합니다."
			>
