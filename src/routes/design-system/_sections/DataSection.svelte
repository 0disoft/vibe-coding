<script lang="ts">
	...
	const chartDataRows = chartValues.map((v, i) => ({
		id: i,
		month: i + 1,
		value: v,
	}));
	...
	<DsCard class="space-y-6">
					...
					{#snippet bottom()}
						<div class="flex items-center justify-between">
							<span class="text-helper text-muted-foreground"
								>Updated 2m ago</span
							>
							<DsButton size="sm" variant="ghost" intent="secondary"
								>View</DsButton
							>
						</div>
					{/snippet}
		...
		<div class="space-y-2">
			...
			>
				...
				{#snippet table()}
					<DsDataTable
						caption="Monthly revenue values"
						columns={[
							{ id: "month", header: "Month", sortable: true },
