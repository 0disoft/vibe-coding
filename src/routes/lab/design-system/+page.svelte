<script lang="ts">
	...
	const tokens: TokenRow[] = [
		{
			...
			sampleBgClass: "bg-background",
			sampleTextClass: "text-foreground",
		},
		{
			name: "Text",
			cssVar: "--color-text",
			sampleBgClass: "bg-foreground",
			sampleTextClass: "text-background",
		},
		{
			name: "Surface",
			cssVar: "--color-surface",
			sampleBgClass: "bg-card",
			sampleTextClass: "text-card-foreground",
		},
		{
			name: "Primary",
			cssVar: "--color-primary",
			sampleBgClass: "bg-primary",
			sampleTextClass: "text-primary-foreground",
		},
		{
			name: "Primary Hover",
		...
		},
		{
			...
			sampleBgClass: "bg-secondary",
			sampleTextClass: "text-secondary-foreground",
		},
		{
			name: "Muted Text",
			cssVar: "--color-text-muted",
			sampleBgClass: "bg-muted",
			sampleTextClass: "text-muted-foreground",
		},
		{
			name: "Accent",
			cssVar: "--color-accent",
			sampleBgClass: "bg-accent",
			sampleTextClass: "text-accent-foreground",
		},
		{
			name: "Link",
			cssVar: "--color-link",
			sampleBgClass: "bg-link",
			sampleTextClass: "text-link-foreground",
		},
		{
			name: "Border",
			cssVar: "--color-border",
			sampleBgClass: "bg-border",
			sampleTextClass: "text-background",
		},
		{
			name: "Input",
			cssVar: "--color-input",
			sampleBgClass: "bg-input",
			sampleTextClass: "text-background",
		},
		{
			name: "Error",
			cssVar: "--color-error",
			sampleBgClass: "bg-destructive",
			sampleTextClass: "text-destructive-foreground",
		},
		{
			name: "Success",
			cssVar: "--color-success",
			sampleBgClass: "bg-success",
			sampleTextClass: "text-success-foreground",
		},
		{
			name: "Warning",
			cssVar: "--color-warning",
			sampleBgClass: "bg-warning",
			sampleTextClass: "text-warning-foreground",
		},
		{
			name: "Selected",
			cssVar: "--color-selected",
			sampleBgClass: "bg-selected",
			sampleTextClass: "text-selected-foreground",
		},
		{
			name: "Sidebar",
			cssVar: "--color-sidebar",
			sampleBgClass: "bg-sidebar",
			sampleTextClass: "text-sidebar-foreground",
		},
		{
			name: "Overlay",
			cssVar: "--color-overlay",
			sampleBgClass: "bg-overlay",
			sampleTextClass: "text-foreground",
		},
		{
			name: "Surface Hover",
	...
	const radiusVars = ["--radius-sm", "--radius-md", "--radius-lg"] as const;

	const borderVars = ["--border-width"] as const;

	const opacityVars = ["--opacity-disabled", "--opacity-hover"] as const;
...
<div class="space-y-10 py-10">
	<header class="space-y-2">
		...
		<p class="text-body-secondary text-muted-foreground">
			이 페이지는 DEV 환경에서만 열립니다. 토큰/유틸리티/상태 UI를 한 곳에서
			빠르게 검증하는 용도입니다.
		</p>
	</header>
	...
	<section class="space-y-4">
		...
		<p class="text-body-secondary text-muted-foreground">
			아래 두 패널은 전역 테마와 무관하게 <code class="text-inline-code"
				>.ds-lab</code
			>
			스코프 토큰으로 렌더링됩니다. 가이드 네이밍(<code class="text-inline-code"
				>--color-*</code
			>)을 canonical로 두고, 기존 변수는 alias로 유지합니다.
		</p>

		<div class="grid gap-6 lg:grid-cols-2">
			<div
				bind:this={lightEl}
				class="ds-lab rounded-xl border border-border bg-background p-5 text-foreground"
				data-ds-theme="light"
			>
				<div class="mb-4 flex items-center justify-between">
					<h3 class="text-h3 font-semibold">Light</h3>
					<span class="text-caption text-muted-foreground"
						>data-ds-theme="light"</span
					>
				</div>
						...
						<div class="rounded-lg border border-border bg-card p-3">
							<div class="flex items-start justify-between gap-3">
								...
								{#if t.sampleBgClass && t.sampleTextClass}
									<div
										class={`h-9 w-9 rounded-md border border-border ${t.sampleBgClass} flex items-center justify-center`}
									>
										<span class={`text-badge font-bold ${t.sampleTextClass}`}
											>A</span
										>
									</div>
				...
				<div class="mt-6 space-y-4">
								...
								<div class="mt-3 space-y-2">
									...
									<div class="grid gap-2 sm:grid-cols-5">
										...
										<div class="rounded-lg border border-border bg-card p-3">
											...
											<div class="mt-2">
												<DsButton intent="primary" data-ds-state="hover"
													>Primary</DsButton
												>
											</div>
										</div>
										<div class="rounded-lg border border-border bg-card p-3">
											...
											<div class="mt-2">
												<DsButton intent="primary" data-ds-state="focus"
													>Primary</DsButton
												>
											</div>
										</div>
										<div class="rounded-lg border border-border bg-card p-3">
											...
											<div class="mt-2">
												<DsButton intent="primary" data-ds-state="active"
													>Primary</DsButton
												>
											</div>
										</div>
						...
						<div class="space-y-2">
							...
							<div class="space-y-2">
								<label class="block space-y-1">
									...
									<div class="grid gap-2">
										<DsInput
											size="sm"
											type="email"
											placeholder="you@example.com (sm)"
										/>
								...
								<label class="block space-y-1">
									...
									<div class="text-helper text-destructive">
										유효한 이메일 주소를 입력하세요.
									</div>
								</label>
								...
								<div class="mt-4 space-y-2">
									...
									<div class="grid gap-3">
										<DsField
											id="lab-light-email"
											label="Email"
											helpText="이메일로 로그인 링크를 보낼 수 있어요."
											required
										>
											{#snippet children(p)}
												<DsInput
					...
					<DsCard motion>
						<div class="flex items-center justify-between gap-3">
							...
							<span
								class="text-badge rounded-full bg-selected px-2 py-0.5 text-selected-foreground"
								>Selected</span
							>
						</div>
						<div class="mt-3 text-body">
							<a
								class="ds-focus-ring text-link underline underline-offset-2 hover:text-link/90"
								href="/lab/design-system">링크 토큰 테스트</a
							>
						</div>
					</DsCard>
					...
					<div class="space-y-2">
						...
						<div class="grid gap-2 sm:grid-cols-2">
							{#each a11yVars as name (name)}
								<div class="rounded-lg border border-border bg-card p-3">
									...
									<div class="mt-1 text-helper text-muted-foreground font-mono">
										{lightVars[name] ?? "..."}
									</div>
								</div>
							{/each}
						...
						<div class="flex flex-wrap gap-2">
							...
							<span class="text-helper text-muted-foreground">
								아이콘 버튼은 <code class="text-inline-code"
									>--touch-target-min</code
								> 이상을 목표로 합니다.
							</span>
						</div>
					</div>
								...
								<div class="grid gap-2 sm:grid-cols-2">
									{#each spacingVars as name (name)}
										<div class="rounded-lg border border-border bg-card p-3">
											...
											<div
												class="mt-2 rounded-md border border-border bg-background"
												style={`padding: var(${name});`}
											>
												<div
													class="h-6 w-6 rounded bg-selected"
													aria-hidden="true"
												></div>
											</div>
											<div
												class="mt-2 text-helper text-muted-foreground font-mono"
											>
												{lightVars[name] ?? "..."}
											</div>
										</div>
						...
						<div class="grid gap-4 lg:grid-cols-2">
							<div class="space-y-2">
								...
								<div class="grid gap-2 sm:grid-cols-3">
									<div
										class="ds-elevation-1 rounded-lg border border-border bg-background p-3"
									>
										<div class="text-helper text-muted-foreground">
											Elevation 1
										</div>
									...
									<div
										class="ds-elevation-2 rounded-lg border border-border bg-background p-3"
									>
										<div class="text-helper text-muted-foreground">
											Elevation 2
										</div>
									...
									<div
										class="ds-elevation-3 rounded-lg border border-border bg-background p-3"
									>
										<div class="text-helper text-muted-foreground">
											Elevation 3
										</div>
							...
							<div class="space-y-2">
								...
								<div
									class="relative h-36 overflow-hidden rounded-lg border border-border bg-card p-3"
								>
									<div
						...
						<div class="grid gap-4 lg:grid-cols-2">
							<div class="space-y-2">
								...
								<div class="flex flex-wrap gap-2">
									<DsButton
										intent="primary"
										onclick={() => (dialogOpenLight = true)}
										>Open dialog</DsButton
									>
								</div>
								...
								>
									<p class="text-body">
										이 Dialog는 <code class="text-inline-code"
											>&lt;dialog&gt;</code
										> 기반이며, ESC/바깥 클릭으로 닫힙니다.
									</p>
								</DsDialog>
							</div>
							...
							<div class="space-y-2">
								...
								<div class="flex items-center gap-3">
									...
									/>
									<span class="text-helper text-muted-foreground">
										Last: <code class="text-inline-code"
											>{lastDropdownLight ?? "-"}</code
										>
									</span>
								</div>
							</div>
								...
								<div class="grid gap-3 sm:grid-cols-2">
									...
									<div class="rounded-lg border border-border bg-card p-3">
										...
										<div class="mt-2 flex flex-wrap items-center gap-2">
											<DsTooltip
												content="키보드 포커스/호버에서 열리는 툴팁입니다."
												placement="top"
											>
												{#snippet children(p)}
													<button
