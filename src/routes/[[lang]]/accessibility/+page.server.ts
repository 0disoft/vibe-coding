...
const accessibilityFiles = import.meta.glob('/src/content/accessibility/*.md', {
	query: '?raw',
	import: 'default',
	eager: true
...
export const load: PageServerLoad = ({ url }) => {
	...
	const getFilePath = (locale: string) => `/src/content/accessibility/${locale}.md`;

	let markdown = accessibilityFiles[getFilePath(lang)];
	let actualLang = lang;

	// 해당 언어 파일이 없으면 영어로 fallback
	if (!markdown) {
		markdown = accessibilityFiles[getFilePath('en')];
		actualLang = 'en';

		// 영어 파일조차 없으면 500 에러
		if (!markdown) {
			throw error(500, 'Accessibility statement content not found');
		}
	}
